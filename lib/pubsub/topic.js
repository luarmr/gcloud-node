 ...
 * });
 */
function Topic(pubsub, options, fullname) {
  this.makeReq_ = pubsub.makeReq_.bind(pubsub);
  
  this.name = fullname ? options.name 
                       : Topic.formatName_(pubsub.projectId, options.name);
...
Topic.prototype.subscribe = function(name, options, callback) {
  ...
  if (options.pushConfig) {
    body.pushConfig = options.pushConfig;
  }
